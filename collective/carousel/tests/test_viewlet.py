...
class ViewletTestCase(TestCase):
    ...
    def test_viewlet_rendering(self):
        ...
        field = self.folder.Schema().getField('carouselprovider')
        # technically the following checkup is done in test_field, but we
        # better check again
        ...
        for result in viewlet.results(viewlet.getProviders()[0]):
            ...
            if item_type == 'Document':
                self.failUnless('<p>This is a PAGE tile</p>' in
                                viewlet.get_tile(result.getObject()))
            if item_type == 'Event':
                self.failUnless('<p>This is a DEFAULT tile</p>' in
                                viewlet.get_tile(result.getObject()))
            if item_type == 'News Item':
                self.failUnless('<p>This is a NEWS ITEM tile</p>' in
                                viewlet.get_tile(result.getObject()))
        ...
        self.failIf('<p>This is a CUSTOM DEFAULT tile</p>' in
                    viewlet.get_tile(event))
    ...
    def test_edit_carousel_link(self):
        ...
        # Check whether anonymous users get "edit" link. First check that
        # Manager gets it:
